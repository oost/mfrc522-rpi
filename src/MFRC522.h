 ...
 *(access.thing.dk), fall of 2013 (Translation to English, refactored, comments,
 *anti collision, cascade levels.) Extended by Tom Clement with functionality to
 *write to sector 0 of UID changeable Mifare cards. Released into the public
 *domain.
 *
 ...
 *http://www.mouser.com/ds/2/302/MF1S503x-89574.pdf 4K:
 ...
 *http://www.mouser.com/ds/2/302/MF1S503x-89574.pdf sections 8.6 and 8.7: Bytes
 *0-5:   Key A Bytes 6-8:   Access Bits Bytes 9:     User data Bytes 10-15: Key
 *B (or user data) Block 0 is read-only manufacturer data. To access a block, an
 *authentication using a key from the block's sector must be performed first.
 * 		Example: To read from block 10, first authenticate using a key
 *from sector 3 (blocks 8-11). All keys are set to FFFFFFFFFFFFh at chip
 *delivery. Warning: Please read section 8.7 "Memory Access". It includes this
 *text: if the PICC detects a format violation the whole sector is irreversibly
 *blocked. To use a block in "value block" mode (for Increment/Decrement
 *operations) you need to change the sector trailer. Use PICC_SetAccessBits() to
 *calculate the bit patterns. MIFARE Classic 4K (MF1S703x): Has (32 sectors * 4
 *blocks/sector + 8 sectors * 16 blocks/sector) * 16 bytes/block = 4096 bytes.
...
public:
  ...
  enum PICC_Command : byte {
    ...
    PICC_CMD_MF_READ =
        0x30, // Reads one 16 byte block from the authenticated sector of the
  ...
  // more. last value set to 0xff, then compiler uses less ram, it seems some
  ...
  // GetStatusCodeName() if you add more. last value set to 0xff, then compiler
  // uses less ram, it seems some optimisations are triggered
  ...
  typedef struct {
    ...
    byte uidByte[10];
    byte sak; // The SAK (Select acknowledge) byte returned from the PICC after
  ...
  StatusCode PCD_CommunicateWithPICC(byte command, byte waitIRq, byte *sendData,
                                     byte sendLen, byte *backData = nullptr,
...
protected:
  byte _chipSelectPin; // Arduino pin connected to MFRC522's SPI slave select
                       // input (Pin 24, NSS, active low)
  byte _resetPowerDownPin; // Arduino pin connected to MFRC522's reset and power
                           // down input (Pin 6, NRSTPD, active low)
