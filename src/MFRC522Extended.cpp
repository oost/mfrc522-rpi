 ...
 */
MFRC522::StatusCode MFRC522Extended::PICC_Select(
  ...
  // We also check SAK bit 3 is cero, as it stands for UID complete (1 would
  // tell us it is incomplete)
    ...
    if (result == STATUS_OK) {
      ...
      if (ats.size > 0) {
        ...
        if (ats.ta1.transmitted) {
          ...
          // ---+---+---+---+---+---+---+---+------------------------------------------
          //  0 | - | - | - | 0 | - | - | - | Different D for each direction
          //  supported 1 | - | - | - | 0 | - | - | - | Only same D for both
          //  direction supported
          //  - | x | x | x | 0 | - | - | - | DS (Send D)
...
MFRC522::StatusCode MFRC522Extended::PICC_RequestATS(Ats *ats) {
  ...
  // The CID defines the logical number of the addressed card and has a range of
  // 0 through 14; 15 is reserved for future use (RFU).
  //
  ...
  result =
      PCD_TransceiveData(bufferATS, 4, bufferATS, &bufferSize, NULL, 0, true);
  if (result != STATUS_OK) {
    PICC_HaltA();
  }
  ...
  if (ats->size > 0x01) {
    ...
    case 0x08:
      ...
      // The reason I ignore it is that MFRC255 FIFO is 64 bytes so this is not
      // a possible value (or atleast it shouldn't)
      // ats->fsc = 256;
      break;
    ...
    if (ats->tb1.transmitted) {
      ...
      ats->tb1.sfgi = 0; // The default value of SFGI is 0 (meaning that the
    ...
    ats->tb1.transmitted = false;
    ats->tb1.fwi = 0;  // TODO: Don't know the default for this!
    ats->tb1.sfgi = 0; // The default value of SFGI is 0 (meaning that the card
                       // does not need any particular SFGT)
...
MFRC522::StatusCode MFRC522Extended::PICC_PPS() {
  ...
  //  –The upper nibble(b8–b5) is set to’D'to identify the PPS. All other values
  //  are RFU. -The lower nibble(b4–b1), which is called the ‘card identifier’
  //  (CID), defines the logical number of the addressed card.
  ...
  // Transmit the buffer and receive the response, validate CRC_A.
  result = PCD_TransceiveData(ppsBuffer, 4, ppsBuffer, &ppsBufferSize, NULL, 0,
                              true);
  if (result == STATUS_OK) {
    // Enable CRC for T=CL
...
MFRC522::StatusCode MFRC522Extended::PICC_PPS(TagBitRates sendBitRate,   ///< DS
  ...
  // Bit 8 - Set to '0' as MFRC522 allows different bit rates for send and
  // receive Bit 4 - Set to '0' as it is Reserved for future use.
  // ppsBuffer[2] = (((sendBitRate & 0x03) << 4) | (receiveBitRate & 0x03)) &
  // 0xE7;
  ...
  // Transmit the buffer and receive the response, validate CRC_A.
  result = PCD_TransceiveData(ppsBuffer, 5, ppsBuffer, &ppsBufferSize, NULL, 0,
                              true);
  if (result == STATUS_OK) {
    ...
    if ((ppsBufferSize == 3) && (ppsBuffer[0] == 0xD0)) {
      ...
      // PCD_WriteRegister(RxThresholdReg, 0x84); // ISO-14443.4 Type A
      // (default) PCD_WriteRegister(ControlReg, 0x10);
...
MFRC522::StatusCode MFRC522Extended::TCL_Transceive(PcbBlock *send,
  ...
  if ((txModeReg & 0x80) != 0x80) {
    // Calculate CRC_A
    result = PCD_CalculateCRC(outBuffer, outBufferOffset,
  ...
  result =
      PCD_TransceiveData(outBuffer, outBufferOffset, inBuffer, &inBufferSize);
  if (result != STATUS_OK) {
    return result;
  }
  ...
  if ((rxModeReg & 0x80) != 0x80) {
    ...
    MFRC522::StatusCode status =
        PCD_CalculateCRC(inBuffer, inBufferSize - 2, controlBuffer);
    if (status != STATUS_OK) {
      return status;
    }

    if ((inBuffer[inBufferSize - 2] != controlBuffer[0]) ||
        (inBuffer[inBufferSize - 1] != controlBuffer[1])) {
      return STATUS_CRC_WRONG;
    }
  ...
  if (inBufferSize > inBufferOffset) {
    ...
    memcpy(back->inf.data, &inBuffer[inBufferOffset],
           inBufferSize - inBufferOffset);
 ...
 */
MFRC522::StatusCode
MFRC522Extended::TCL_Transceive(TagInfo *tag, byte *sendData, byte sendLen,
...
MFRC522::StatusCode MFRC522Extended::TCL_Deselect(TagInfo *tag) {
  ...
  result =
      PCD_TransceiveData(outBuffer, outBufferSize, inBuffer, &inBufferSize);
  if (result != STATUS_OK) {
    return result;
  }

  // TODO:Maybe do some checks? In my test it returns: CA 00 (Same data as I
  // sent to my card)
 ...
 * @return PICC_Type
 */
MFRC522::PICC_Type MFRC522Extended::PICC_GetType(
    TagInfo *tag ///< The TagInfo returned from PICC_Select().
) {
 ...
 */
void MFRC522Extended::PICC_DumpDetailsToSerial(
    TagInfo *tag ///< Pointer to TagInfo struct returned from a successful
                 ///< PICC_Select().
) {
  // ATQA
  ...
  Serial.println(F(PICC_GetTypeName(piccType)));
...
void MFRC522Extended::PICC_DumpISO14443_4(TagInfo *tag) {
  // ATS
  if (tag->ats.size >
      0x00) { // The first byte is the ATS length including the length byte
