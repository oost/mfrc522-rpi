...
char SERIAL_DRIVER_NAME[128] = "/dev/tty";
...
void SerialLinux::begin(const char *serialPort, int baud,
  ...
  (ParityEN) ? options.c_cflag |= PARENB
             : options.c_cflag &= ~PARENB; // Parity enable ? YES : NO
  (Parity) ? options.c_cflag |= PARODD
           : options.c_cflag &= ~PARODD; // Parity ? Odd : Even
  (StopBits) ? options.c_cflag |= CSTOPB
             : options.c_cflag &= ~CSTOPB; // Stop bits ? 2 bits: 1 bit
...
int SerialLinux::availableForWrite() { return 63; }

bool SerialLinux::find(const char *target) { return findUntil(target, NULL); }

// Reads data from the serial buffer until a target string of given length,
// terminator string is found or times out.
// Returns: true if target string is found, false if times out or terminator is
// found.
...
void SerialLinux::flush() { tcflush(fd, TCIOFLUSH); }
...
size_t SerialLinux::print(char c) { return unistd::write(fd, &c, 1); }
...
size_t SerialLinux::readBytesUntil(char terminator, char buffer[],
                                   size_t length) {
  timespec time1, time2;
  clock_gettime(CLOCK_REALTIME, &time1);
  size_t count = 0;
...
// Reads a string unitl a termintor is given, this function blocks until the
// terminator is found. Terminator character is not added to the char array and
// last character of the array is always terminated with a null ('\0') character
size_t SerialLinux::readStringCommand(char terminator, char buffer[],
                                      size_t length) {
  size_t count = 0;
  char c;
  if (length <= 0)
...
void SerialLinux::setTimeout(long millis) { timeOut = millis; }
