...
public:
  ...
  typedef struct {
    ...
    // Raw data from ATS
    byte data[FIFO_SIZE - 2]; // ATS cannot be bigger than FSD - 2 bytes (CRC),
  ...
  StatusCode PICC_PPS(TagBitRates sendBitRate,
                      TagBitRates receiveBitRate); // Different D values
  ...
  StatusCode TCL_TransceiveRBlock(TagInfo *tag, bool ack, byte *backData = NULL,
                                  byte *backLen = NULL);
  ...
  using MFRC522::PICC_GetType; // // make old PICC_GetType(byte sak) available,
                               // otherwise would be hidden by
                               // PICC_GetType(TagInfo *tag)

  // Support functions for debuging
  void PICC_DumpToSerial(TagInfo *tag);
  using MFRC522::PICC_DumpToSerial; // make old PICC_DumpToSerial(Uid *uid)
                                    // available, otherwise would be hidden by
                                    // PICC_DumpToSerial(TagInfo *tag)
  void PICC_DumpDetailsToSerial(TagInfo *tag);
  using MFRC522::
      PICC_DumpDetailsToSerial; // make old PICC_DumpDetailsToSerial(Uid *uid)
                                // available, otherwise would be hidden by
                                // PICC_DumpDetailsToSerial(TagInfo *tag)
